...
-- 4) RECAP PHOTOS (storage bucket + policies + event_photos table) -----------
INSERT INTO storage.buckets (id, name, public)
VALUES ('event-images', 'event-images', true)
ON CONFLICT (id) DO UPDATE SET public = true;

DROP POLICY IF EXISTS "event-images public read" ON storage.objects;
CREATE POLICY "event-images public read" ON storage.objects FOR SELECT TO public
...
CREATE POLICY "event_photos read" ON public.event_photos FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "event_photos insert own" ON public.event_photos;
CREATE POLICY "event_photos insert own" ON public.event_photos FOR INSERT TO authenticated
  WITH CHECK (uploader_id = (select auth.uid()));
DROP POLICY IF EXISTS "event_photos delete" ON public.event_photos;
CREATE POLICY "event_photos delete" ON public.event_photos FOR DELETE TO authenticated
