/*
  # Per-topic / per-event notification subscriptions

  ## Why
  Users want to control notifications per item:
  - Topics: default OFF, EXCEPT the topic you authored (your question) which
    is ON by default. Anyone can follow a topic to get replies, and the
    author can mute their own.
  - Events: default ON (you RSVP'd, you want updates). Attendees can mute a
    specific event.

  ## What
  - notification_subscriptions(user_id, target_type, target_id, subscribed):
    an explicit override row. No row = fall back to the default for that type.
  - notify_topic_comment rewritten: recipients = topic author (unless a mute
    row exists) UNION explicit subscribers (subscribed = true), minus the
    commenter.
  - notify_event_announcement (new): notify every registered attendee except
    the author, unless they have a mute row (subscribed = false).
*/
...
-- ----------------------------------------------------------------------------
-- Topic comments: author (default on, mutable) + explicit followers.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    r.uid,
    'comment',
    COALESCE(v_commenter_name, 'Someone') || ' commented on '
      || CASE WHEN r.uid = v_topic_author THEN 'your post' ELSE 'a post you follow' END,
    LEFT(COALESCE(NEW.content, ''), 140),
    jsonb_build_object('topic_id', NEW.topic_id, 'comment_id', NEW.id, 'user_id', NEW.user_id)
  FROM (
    -- The author is subscribed by default...
    SELECT v_topic_author AS uid WHERE v_topic_author IS NOT NULL
    UNION
    -- ...plus anyone who explicitly followed this topic.
    SELECT ns.user_id
  ...
  WHERE r.uid <> NEW.user_id
    -- ...unless they've muted this topic.
    AND NOT EXISTS (
      SELECT 1 FROM public.notification_subscriptions m
      WHERE m.user_id = r.uid AND m.target_type = 'topic'
        AND m.target_id = NEW.topic_id AND m.subscribed = false
    );
...
-- ----------------------------------------------------------------------------
-- Event announcements: all registered attendees (default on), mutable.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ea.user_id,
    'event_update',
    'Update for ' || COALESCE(v_event_title, 'an event'),
    LEFT(COALESCE(NEW.content, ''), 140),
  ...
  WHERE ea.event_id = NEW.event_id
    ...
    AND ea.user_id <> NEW.author_id
    AND NOT EXISTS (
      SELECT 1 FROM public.notification_subscriptions m
      WHERE m.user_id = ea.user_id AND m.target_type = 'event'
        AND m.target_id = NEW.event_id AND m.subscribed = false
    );
...
CREATE TRIGGER event_announcement_notification
