/*
  ...
  event that's coming up. Two reminder windows:
  ...
  - send_event_reminders() SECURITY DEFINER function that scans events
    starting within ~1h and ~24h windows and inserts notifications. Uses
...
DECLARE
  ...
  v_body text;
BEGIN
  -- Iterate over events whose start time falls in either reminder window.
  ...
  LOOP
    -- Decide which window applies (if any). Allow a 15-minute grace so
    ...
    FOR v_recipient IN
      ...
      WHERE uid IS NOT NULL
        AND NOT EXISTS (
          ...
          WHERE r.event_id = v_event.id AND r.user_id = uid AND r.window = v_window
        )
    LOOP
      ...
      VALUES (
        ...
        v_title,
        v_body,
        jsonb_build_object('event_id', v_event.id, 'window', v_window)
      );
      INSERT INTO public.event_reminders_sent (event_id, user_id, window)
      VALUES (v_event.id, v_recipient, v_window);
