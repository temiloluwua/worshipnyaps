/*
  # Initial Schema for Calgary Bible Study Community App - FIXED
  ...
  2. Security
    ...
    - FIXED: Removed insecure USING (true) policies
...
-- Locations policies
CREATE POLICY "Authenticated users can read approved locations"
  ON locations FOR SELECT
  TO authenticated
  USING (is_approved = true OR host_id = auth.uid());

CREATE POLICY "Hosts can create locations"
...
CREATE POLICY "Users can read attendees of events they're involved in"
...
CREATE POLICY "Users can read worship songs for events they're involved in"
  ...
  USING (
    EXISTS (
      ...
      AND (
        ...
        OR events.worship_leader_id = auth.uid()
        OR EXISTS (
          SELECT 1 FROM event_attendees
          WHERE event_attendees.event_id = events.id
          AND event_attendees.user_id = auth.uid()
...
CREATE POLICY "Event hosts and worship leaders can manage songs"
...
CREATE POLICY "Event hosts and worship leaders can delete songs"
...
-- Food items policies
CREATE POLICY "Users can read food items for events they're involved in"
  ...
  USING (
    EXISTS (
      ...
      AND (
        events.host_id = auth.uid()
        OR events.hospitality_coordinator_id = auth.uid()
        OR EXISTS (
          SELECT 1 FROM event_attendees
          WHERE event_attendees.event_id = events.id
          AND event_attendees.user_id = auth.uid()
...
CREATE POLICY "Event hosts and hospitality coordinators can manage food"
...
CREATE POLICY "Event hosts and hospitality coordinators can delete food"
