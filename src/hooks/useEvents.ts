...
// Cap so a far-off end date can never spawn a runaway number of rows.
...
export const recurrenceDatesUntil = (startISO: string, recurrence: Recurrence, untilISO: string): string[] => {
  ...
  for (let i = 0; i < MAX_OCCURRENCES; i += 1) {
    ...
    if (recurrence === 'weekly') d.setDate(start.getDate() + 7 * i);
    else if (recurrence === 'biweekly') d.setDate(start.getDate() + 14 * i);
